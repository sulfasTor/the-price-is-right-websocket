function GameWorld(){
	...
	$('#btn_submit_answer').click(function(){
	    if(!answerSubmitted && ((userType=='official_participant' || userType=='unofficial_participant') && (curState==states.SHOW_QUESTION || curState==states.TEST_QUESTION))){
		answer = $('#answer_id').val();
                if (!isNaN(parseFloat(answer)) && isFinite(answer)) {
                    answerSubmitted = true;
                    socket.emit('quiz_send_answer',{answerId:answer});
                } else {
                    alert('Answer is not valid, should be a number!');
                }
	    }
	});
...
this.showAnswer = function(stateParams){
	...
	var proof = stateParams.proof;
	var winner = stateParams.winner;
