		...
		if(fs.existsSync(path)){
			...
			if(line == sQuestionEnd){
				...
				console.log("# Invalid marks: "+line);
                        ...
                        else if(line.startsWith(sProof)){
			    if(currentQuestion){
				currentQuestion.setProof(line.substr(sProof.length));
			    }
                        }
...
function Participants(){
    ...
    this.updateScores = function(answerId,marks){
        var realParticipantsIds = [];
        for (var id in participants){
            if (participants[id] && participants[id].isRealParticipant) {
                realParticipantsIds.push(id);
            }
        }

        var winner = realParticipantsIds[0];
        var diff = Math.abs (answerId - participants[winner].getResponse());
	realParticipantsIds.forEach(function(i) {
            participants[i].updateScore(false, 0); // Initial score
            var newdiff = Math.abs(answerId - participants[i].getResponse());
            if (newdiff < diff) {
                diff = newdiff;
                winner = i;
            }
        ...
        var p = participants[winner];
        p.updateScore(true, marks);
...
function RealParticipant(pSocket,pTeamname){
    ...
    //TODO: Update answer validation
    this.updateScore = function(winner, marks){
	if(winner){
	    score += marks;
    ...
    //TODO: Update answer
    this.revertScore = function(winner,marks){
	if(winner){
	    score -= marks;
	}
    }
...
function Question(){
	...
	return {question:question,answers:answers,pic:pic,time:time,marks:marks, proof:proof};
    }
...
function QuizState(pQuizId){
    ...
    this.setQuestionWinner = function(pWinner){
        winner = pWinner;
    }
    ...
    this.getQuestionWinner = function(){
	return winner;
    }
...
function Quiz(pQuizId){
	...
	quizState.setHiddenParams({answerId: testQuestion.getAnswerId(),
                                   test:true,
                                   marks: testQuestion.getMarks(),
                                   proof: testQuestion.getProof()});
	...
	quizState.setHiddenParams({answerId: question.getAnswerId(),
	...
	var url = hiddenParams.proof;

	var winner = participants.updateScores(answerId,marks);

	var params = {answerId:answerId, proof: url, winner: winner};
